...
const EducationLoanComponent = () => {
  const [selectedSubcategory, setSelectedSubcategory] = useState('University Fees');
  const [initialDeposit, setInitialDeposit] = useState(0);  // Initialize with 0
  const [loanPeriod, setLoanPeriod] = useState(12); // Default: 12 months
  ...
  const calculateLoan = () => {
    if (initialDeposit < 0 || initialDeposit > loanCategories[0].maxLoan) {
      alert("Initial deposit must be between 0 and the maximum loan amount.");
      return;
    }
      ...
      <form onSubmit={handleSubmit} className="mt-6 space-y-6">
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => setInitialDeposit(parseInt(e.target.value) || 0)}  // Ensuring valid number
            className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
